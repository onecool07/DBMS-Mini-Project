...
<select>
     ...
     <?php
	  echo "<option value =''>objectid.Id..</option>";
...
//	   $scientistid = $_POST['scientistid'];
    ...
    // mysql search query
    $up = "delete from `celestia` 
	 where `objectid`= '$objectid'";
   $sel="SELECT objectid, name, mass,date,type,scientistid FROM celestia";
...
echo "<tr><td>" . $row["objectid"] . "</td><td>" . $row["name"] . "</td><td>" . $row["mass"] . "</td><td>" . $row["date"] .  "</td><td>" . $row["type"] .   "</td><td>" . $row["scientistid"] . "</td></tr>";
}} else {
    echo "0 results";
}
